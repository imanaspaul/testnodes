...
## ![Django ecommerce tutorial from manascode.com](https://manascode.com/wp-content/uploads/2019/08/django-ecomerce-website-tutorial-manascode-1024x575.jpg)<figcaption>Home Page View</figcaption></figure>8. Adding Products to Cart
...
if order_qs.exists():
order = order_qs[0] # check if the order item is in the order
if order.orderitems.filter(item\_\_slug=item.slug).exists():
...
if cart_qs.exists():
cart = cart_qs[0] # Checking the cart quantity
if cart.quantity > 1:
cart.quantity -= 1
cart.save()
else:
