...
<div class="hcb_wrap">```
<pre class="prism line-numbers lang-python" data-lang="Python" data-line="CartView.py">```
# Cart View
...
</div>we can see that now we need to create a new Template for the CART view page called home.html . So again the same steps as we did in the last part. Create a new folder in the cart app called `templates` then create another folder inside the new templates folder called as same as your app name. Now create the `home.html` file inside the folder.
...
<pre class="prism line-numbers lang-python" data-lang="Python" data-line="custom template tag for counting the cart item">```
{{ request.user| cart_total }}
```
```

</div>### Creating the cart home page.
